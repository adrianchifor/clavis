...
def get_or_create_rpc_security(rpc_sec_path: str) -> str:
    if not k8s_namespace:
        logger.error(
            "Failed to create/get k8s TLS secret, cannot get clavis server namespace"
        )
    ...
    try:
        secret = (
            ...
            .filter(namespace=k8s_namespace)
            .get_or_none(name="clavis-tls")
        )
        if not secret:
            ...
            secret = {
                "apiVersion": "v1",
                "kind": "Secret",
                "metadata": {"name": "clavis-tls", "namespace": k8s_namespace},
                "type": "Opaque",
                "data": {
            ...
            integrity_secret = base64.b64decode(secret.obj["data"]["integrity"]).decode(
                "utf-8"
            )
